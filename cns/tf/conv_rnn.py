...
class Config(object):
    def __init__(self, **kwargs):
        self.seq_size = kwargs.get("seq_size", 1000)
        self.batch_size = kwargs.get("batch_size", 1)
        self.filter_len = kwargs.get("filter_len", 100)
...
recov_init = tf.uniform_unit_scaling_initializer(factor=5.0)
...
def le(state_v, inputs_v):
    return sess.run(
        [
            cells._cells[len(cells._cells)/2].states_info,
            finstate,
        ], 
        {
            input: inputs_v.reshape(1, c.seq_size, 1, 1),
...
def generate_le(data):
    state_v = np.zeros((c.batch_size, cells.state_size))

    conv_data = []
    for start_id in np.arange(0, data.shape[0]-c.seq_size, c.seq_size):
        conv_data.append(
            np.squeeze(
                run_conv(
                    data[start_id:(start_id+c.seq_size)]
                )
            )
        )
        
    conv_data = np.concatenate(conv_data)

    input_c = [ tf.placeholder(tf.float32, shape=(c.batch_size, c.filters_num), name="InputToNet{}".format(ii)) for ii in xrange(c.seq_size) ]

    vs.get_variable_scope().reuse_variables()
    r, finstate = rnn.rnn(cells, input_c, initial_state=state)

    le_data = []
    for start_id in np.arange(0, conv_data.shape[0]-c.seq_size, c.seq_size):
        feed_dict = {k: v for k, v in zip(
            input_c, 
            [ np.expand_dims(conv_data[start_id + sid, :], 0) for sid in xrange(c.seq_size) ]
        )}
        feed_dict[state] = state_v

        ret = sess.run(
            [r, finstate] + cells._cells[len(cells._cells)/2].states_info, 
            feed_dict
        )
        _, state_v = re[0], ret[1]

        le_data += ret[2:]

    return np.concatenate(le_data)
