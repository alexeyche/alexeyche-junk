...
run_srm <- function(net_neurons, net, ro) {  
  ...
  for(sim_time0 in sim_times) {
    if(sim_time0<ro$Tmax) {
        ...
        if (ro$mode == "learn") {
          gr = net_neurons$l[[ro$learn_layer_id]]$grad(sim_time0, sim_time0+ro$learn_window_size, net, ro$target_set)
            
          invisible(sapply(1:net_neurons$l[[ro$learn_layer_id]]$len, function(i) {
            net_neurons$l[[ro$learn_layer_id]]$weights[[i]] = net_neurons$l[[ro$learn_layer_id]]$weights[[i]] + ro$learning_rate * gr[[i]]  - ro$weight_decay * net_neurons$l[[ro$learn_layer_id]]$weights[[i]]
          }
          ))
  ...
  if(ro$collect_stat) {
    return(list(net, net_neurons, sim_out$stat, grad=apply(gradients, c(1,2), mean)))
  } else {
