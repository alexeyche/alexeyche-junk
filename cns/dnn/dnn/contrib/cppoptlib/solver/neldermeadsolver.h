 ...
 public:
  ...
  void minimize(Problem<T> &objFunc, Vector<T> & x) {
    ...
    while (iter < maxIter) {
      ...
      T max2 = arma::abs(x0.col(index[1]) - x0.col(index[0]) ).max();
      for (int i = 2; i < DIM + 1; ++i) {
        T tmp1 = fabs(f[index[i]] - f[index[0]]);
        if (tmp1 > max1)
          max1 = tmp1;

        T tmp2 = arma::abs(x0.col(index[i]) - x0.col(index[0]) ).max();
        if (tmp2 > max2)
          max2 = tmp2;
      }
      const T tt1 = std::max(static_cast<T>(1.e-04), 10 * std::nextafter<T>(f[index[0]], std::numeric_limits<T>::epsilon()) - f[index[0]]);
      const T tt2 = std::max(static_cast<T>(1.e-04), 10 * (std::nextafter<T>(x0.col(index[0]).max(), std::numeric_limits<T>::epsilon())
                    - x0.col(index[0]).max()));
