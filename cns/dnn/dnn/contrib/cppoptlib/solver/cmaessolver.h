...
class CMAesSolver : public ISolver<T, 1> {
  ...
  Vector<T> sampleMvn(Vector<T> &mean, Matrix<T> &covar) {
    ...
    Vector<T> stdNormDistr = Zero<T>(mean.n_rows);
    std::normal_distribution<> d(0, 1);
    for (int i = 0; i < mean.n_rows; ++i) {
      stdNormDistr[i] = d(gen);
    }

    Vector<T> samples = normTransform * stdNormDistr + mean;
 ...
 public:
  ...
  void minimize(Problem<T> &objFunc, Vector<T> & x0) {
    ...
    Vector<T> t = Zero<T>(DIM);
    ps = t;
    pc = t;
    Matrix<T> eye = Identity<T>(DIM, DIM);
    C = eye;
    ...
    Vector<T> zeroVectorTemplate = Zero<T>(DIM);
    ...
    for (size_t curIter = 0; curIter < this->settings_.maxIter; ++curIter) {
      ...
      ps = (1. - cs) * ps + sqrt(cs * (2. - cs) * mu_eff) * arma::solve(arma::chol(C).t(), M.step);
      sigma = sigma * pow(  exp((cs / ds * (arma::norm(ps) / ENN - 1.))), 0.3);

      T hs = 0;
      if (arma::norm(ps) / sqrt(pow(1 - (1. - cs), (2 * (curIter + 1)))) < hth)
        hs = 1;
      else
        hs = 0;
      ...
      C = (1 - c1 - cmu) * C + c1 * (pc * pc.t() + delta * C);

      for (int j = 0; j < mu; ++j) {
        C += cmu * w(j) * pop[j].step * pop[j].step.t();
      }

      arma::cx_vec Ec;
      arma::cx_mat Vc;
      arma::eig_gen(Ec, Vc, C);
      Vector<T> E = arma::real(Ec);
      Matrix<T> V = arma::real(Vc);
      ...
      for (int i = 0; i < DIM; ++i) {
        if (E(i) < 0) {
          ...
          pd = false;
          E = ElWiseMax(E, zeroVectorTemplate);
          C = V * E.diag() * V.i();
          break;
        }
      }
      ...
      if ((curIter > 150) && (arma::norm(bestSol.pos-x0) < 1e-8)) {
