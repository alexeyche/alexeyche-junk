...
//        Log::Info << t_left << "|      ";
        for(size_t yi=0; yi<n->y.size(); yi++) {
            ...
            double no_spike = exp(-DEIntegrator<double>::Integrate(n, &prob, t_left, t_right-0.001, 1e-06));
            double spike = 1-exp(-prob(t_right, n));
//            Log::Info << "p:" << no_spike << "     |spike t:" << t_right << " p:" << spike << "|     ";
            p = p*no_spike*spike;
            t_left = t_right+0.001;
        }
        double no_spike = exp(-DEIntegrator<double>::Integrate(n, &prob, t_left, Tmax, 1e-06));
        p = p*no_spike;
