 ...
 */
#ifndef __SIM_CORE_IO_CLI_HPP
#define __SIM_CORE_IO_CLI_HPP
 ...
 *
 * @see sim::CLI, PARAM_FLAG(), PARAM_INT(), PARAM_DOUBLE(), PARAM_STRING(),
 ...
 *     new paragraphs).
 */
#define PROGRAM_INFO(NAME, DESC) static sim::io::ProgramDoc \
    io_programdoc_dummy_object = sim::io::ProgramDoc(NAME, DESC);
 ...
 * http://sim.org/ticket/74 for more information.
 */
 ...
 */
#ifdef __COUNTER__
  #define PARAM(T, ID, DESC, ALIAS, DEF, REQ) static sim::io::Option<T> \
      JOIN(io_option_dummy_object_, __COUNTER__) \
      (false, DEF, ID, DESC, ALIAS, REQ);

  /** @cond Don't document internal macros. */
  #define PARAM_FLAG_INTERNAL(ID, DESC, ALIAS) static sim::io::Option<bool>\
 ...
 * configurability of SIM methods very easy.  There are only three methods in
