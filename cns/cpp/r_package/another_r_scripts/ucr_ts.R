...
read_ts_file <- function(ts_name, sample_size, dir=dir) {
  ...
  if(!is.na(sample_size)) {
    ts_file_train = sprintf("%s/%s/%s_TRAIN_%s",dir, ts_name, ts_name, sample_size)
    ts_file_test = sprintf("%s/%s/%s_TEST_%s",dir, ts_name, ts_name, sample_size)
  } else {
      ts_file_train = sprintf("%s/%s/%s_TRAIN",dir, ts_name, ts_name)
      ts_file_test = sprintf("%s/%s/%s_TEST",dir, ts_name, ts_name)
  }
