...
#define M_CLR   3 /*256  clear table marker */
#define M_EOD   4 /*257  end-of-data marker */
#define M_NEW   5 /*258  new code index */
...
/* encode and decode dictionary structures.
   ...
   or in my case 001:  then dict[0].next[0] = 387, dict[387].next[1] = 1022, etc. */
typedef struct {
    ushort next[2];
} lzw_enc_t;
...
byte* lzw_encode(byte *in, int max_bits)
{
    ...
    for (code = *(in++); --len; ) {
        ...
        printf("c: %d\n",c);
        if ((nc = d[code].next[c])) {
            printf("in dict: nc: %d d[code].next[c]: %d code: %d\n",nc, d[code].next[c], code);
            code = nc;
            printf("now code: %d\n", code);
        }            
        else {
            ...
            printf("next is %d\n", c); 
            printf("nc = d[code].next[c] = next_code++ == %d\n", nc);
            code = c;
        }
    ...
    for(size_t i=0; i<_len(d); i++) {
        printf("%d: d[0]: %d d[1]: %d\n", i, d[i].next[0], d[i].next[1]);
    }
    write_bits(code);
    write_bits(M_EOD);
