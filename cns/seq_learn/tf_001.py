...
def exp_act(u, u_rest=-10.0, gain=1.0):
...
class GLMCell(rc.RNNCell):
    def __init__(self, input_size, hidden_size, visible_size, activation = log_exp_act):
    ...
    def _init_parameters(self):
        if self.W is None:
            self.W = vs.get_variable("W", [self._input_size + self._num_units, self._num_units], initializer=tf.uniform_unit_scaling_initializer(factor=weight_init_factor))
