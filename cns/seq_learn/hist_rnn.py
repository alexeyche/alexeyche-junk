...
def cat_hist(hist, new, dim):
    ...
    return tf.concat_v2([hist_sliced, new], dim)


_RnnHistInputTuple = collections.namedtuple("RnnHistInputTuple", ("input", "target"))

class RnnHistInputTuple(_RnnHistInputTuple):
    __slots__ = ()



def rnn_with_hist_loop_fn(inputs, targets, sequence_length, initial_state, window):
    ...
    target_size = targets.get_shape()[2]

    targets_ta = tf.TensorArray(dtype=tf.float32, size=max_time)
    targets_ta = targets_ta.unstack(targets)
