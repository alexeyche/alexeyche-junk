...
kl_loss = tf.reduce_sum(
    - 0.5 * (
        1.0 +
        2.0 * tf.log(z_sigma) -
        2.0 * tf.log(z_prior_sigma) -
        (tf.exp(2.0 * z_sigma) + tf.square(z_mu - z_prior_mu)) / tf.exp(2.0 * z_prior_sigma)
    ),
    1
)
...
sl(stats[:,0], stats[:,1], stats[:,2], 0.1*stats[:,3], 0.1*stats[:,4], labels=["KL", "mu", "sigma", "recc.loss", "loss"])

with tf.variable_scope("vae", reuse=True) as scope:
    post_mu_v, post_sigma_v = sess.run(
        decode(input_z), {
        input_z: 1.0 * np.random.randn(batch_size, z_dim)
    })
