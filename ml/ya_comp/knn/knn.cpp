...
double* knn(double *t, double *x, int m_test, int m, int n, int k) {
    double *y = new double[m_test];
    double *dists = new double[m];
    for(int cur=0; cur<m_test; cur++) {
        if(cur%10000 == 0) {
            printf("%d lines processed\n", cur);
        }
        double *x_et = splice(t,cur*n,cur*n+n);  // first element 
        for(int i=0; i<m; i++) {
...
int main(int argc, char *argv[]) {
    ...
    int nrow_test = count_rows(test_f);
    int ncol = count_cols(input_f, '\t');
    double *x = read_csv_file(input_f,'\t',nrow,ncol);
    double *t = read_csv_file(test_f,'\t',nrow_test,ncol);
   
    double k1 = 5000;
//    int kn_1 = k1 * sqrt(nrow-1);
    double *y = knn(t, x, nrow_test, nrow, ncol, k1);
