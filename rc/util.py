...
def random_pos_sparse_conn(shape, n_conn):
    p = 1.0 - float(n_conn) / shape[1]
    return random_pos_sparse(shape, p)
...
def rolling_mean(x, steps):
    return pd.Series(x).rolling(steps).mean()
